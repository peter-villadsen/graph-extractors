...
public sealed class Neo4jAdminImportWriter : IGraphWriter
{
    ...
    private string WriteNodeFile(string groupKey, List<CypherNode> nodes)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            fields.AddRange(columns.Select(c => FormatCsvValue(node.Properties.GetValueOrDefault(c), types[c])));
    ...
    private string WriteRelationshipFile(string type, List<CypherRelationship> relationships)
    {
        ...
        foreach (var relationship in relationships)
        {
            ...
            fields.AddRange(columns.Select(c =>
                FormatCsvValue(relationship.Properties is not null && relationship.Properties.TryGetValue(c, out var v) ? v : null, types[c])));
            WriteRow(writer, fields);
        }
        return path;
    }

    /// <summary>Union of property keys across every row in a group, in first-seen order, with a Neo4j type per key inferred from its first non-null value.</summary>
    ...
    private static string FormatCsvValue(object? value, string type)
    {
        if (value is null)
        {
            return "";
        ...
        return type switch
        {
            ...
            "string[]" => string.Join(ArrayDelimiter, ((IEnumerable<object?>)value).Select(v => v?.ToString() ?? "")),
            _ => value.ToString() ?? "",
        };
    }
    ...
    {
        var sb = new StringBuilder("neo4j-admin database import full --multiline-fields=true");
